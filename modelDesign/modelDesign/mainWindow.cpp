...
mainWindow::mainWindow(QWidget *parent) : QMainWindow(parent) {
	...
	//2号
	ui.treeWidget_2->setHeaderLabels(QStringList()<<codec->toUnicode("模型名")<<codec->toUnicode("模型IP")<<codec->toUnicode("模型端口"));
	QTreeWidgetItem* item = new QTreeWidgetItem(QStringList()<<codec->toUnicode("事件管理器")<<"127.0.0.1" <<"12337");
	ui.treeWidget_2->addTopLevelItem(item);
	item = new QTreeWidgetItem(QStringList()<<codec->toUnicode("时间管理器")<<"127.0.0.1" <<"12338");
	ui.treeWidget_2->addTopLevelItem(item);
	//3号
...
void mainWindow::editModelType() {
	...
	if(item) {
		...
		if(mti.isOk) {
			//判断不允许修改
			if((0 == ui.treeWidget_2->currentIndex().row()) && (codec->toUnicode("事件管理器") != mti.nameContent)) {
				WARNING.ui.label->setText(codec->toUnicode("不允许修改！！"));
				WARNING.exec();
				return;
			}
			if((1 == ui.treeWidget_2->currentIndex().row()) && (codec->toUnicode("时间管理器") != mti.nameContent)) {
				WARNING.ui.label->setText(codec->toUnicode("不允许修改！！"));
				WARNING.exec();
				return;
			}
...
void mainWindow::addSubscribe() {
	...
	if(!isHave) {
		WARNING.ui.label->setText(codec->toUnicode("请正确选择模型！！"));
		WARNING.exec();
		return;
	}
	//判断无事件
